...
Get-ADComputer -filter {OperatingSystem -like "*windows server*"} | foreach {
    ...
    if (Test-Connection -ComputerName $server -Quiet -ErrorAction SilentlyContinue -Count 1)
    {
        $results += Get-Certificate -Computername $server -StoreName My -StoreLocation  LocalMachine | 
            select ComputerName, DNSNameList, ExpiresIn, Issuer, `
                   @{Label='SignatureAlgorithm'; Expression={$_.SignatureAlgorithm.FriendlyName}}, Archived 
    }
    else
    {
        Write-Warning "$server does not respond to ping, skipping"
    }

}

$results | Export-CSV -NoTypeInformation $env:TEMP\certs.csv


# open up the temp folder - you want the file certs.csv
