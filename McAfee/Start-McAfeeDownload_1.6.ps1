            ...
            foreach ($WebFile in $WebPageFiles) {
                ...
                try {
                    if (!(Test-Path -Path $NewLocalFile)) {
                        ...
                        Write-Host "`t|-- $strLeaf `t[File exists, checking...]" -ForegroundColor Yellow -NoNewline
                        if ($Proxy -eq '') {
                        ...
                        if ($onlineLength -ne $localLength) {
                            ...
                            # Checked: Date.
                            Write-Host "`t[File older ($onlineModified >= $localModified), downloading...]" -NoNewline #: Online=$onlineLength, Local=$localLength.
                            if ($Proxy -eq '') {
                                Invoke-WebRequest -Uri $NewWebFile -OutFile $NewLocalFile
                            } else {
                                Invoke-WebRequest -proxy $Proxy -Uri $NewWebFile -OutFile $NewLocalFile
