...
if (("-?","-help") -contains $args[0]) {
  Write-Host "[INFO ] Usage: Inventory-DPMActiveLibraries.ps1 [[-DPMServerName] <Name of the DPM server>] "
  Write-Host "[INFO ] Example: Inventory-DPMActiveLibraries.ps1 -DPMServerName ""DPM01"" "
  exit 0
}
...
Write-Host "[INFO ] List all Active Libraries."
$DPMLibraries = Get-DPMLibrary -DPMServerName $DPMServerName
$DPMActiveLibraries = $DPMLibraries | Where-Object -FilterScript {$_.IsOffline -eq 0}
$DPMActiveLibraries

Write-Host "[INFO ] Processing all Active Libraries"
foreach ($DPMLibrary in $DPMActiveLibraries) {
  Write-Host "[INFO ] Processing Library: '$($DPMLibrary.UserFriendlyName)'."
  if ( $DPMLibrary.Type -eq "TapeLibrary") {
    Write-Host "[INFO ] Unlock Library Door"
    $jobList += @{ Job = $DPMLibrary | Unlock-DPMLibraryDoor }

    Write-Host "[INFO ] Wait for process to finish ($secSleepTime sec)."
    sleep $secSleepTime

    Write-Host "[INFO ] Lock Library Door"
    $jobList += @{ Job = $DPMLibrary | Lock-DPMLibraryDoor }

    Write-Host "[INFO ] Wait for process to finish ($secSleepTime sec)."
    sleep $secSleepTime

    Write-Host "[INFO ] Perform Fast Inventory"
    $jobList += @{ Job = $DPMLibrary | Start-DPMLibraryInventory -FastInventory }

    Write-Host "[INFO ] Wait for process to finish ($secSleepTime sec)."
    sleep $secSleepTime
  } else {
    Write-Host "[INFO ] This library is a Stand-Alone drive (No Unlock-Door and Lock-Door required)."
  }
  Write-Host "[INFO ] Schedule Detailed Inventory."
  $jobList += @{ Job = $DPMLibrary | Start-DPMLibraryInventory -DetailedInventory }
}
