#This script removes SCR data source (ExchangeDataSourceConfig) to the PsDatasourceConfig.xml in the current folder

param([string] $source, [string] $sgName)

if(("-?","-help") -contains $args[0])
{
   write-host 
   write-host "Usage::"
   write-host "remove-SCRSG.ps1  [source FQDN]  [SGName]"
   Write-host 
   Write-host "Example:"
   Write-host "remove-SCRSG.ps1 Sellak-ccr-mbox.sellak-ccr.dpmv2.office testSG "
   Write-host 
   exit 0
} 
...
if(Test-Path $configFilePath) {
	...
	write-host -foregroundcolor Red "$configFilePath file not found."
        exit 1;
}
if(!$psDSConfigDoc) {
...
if(!($psDSConfigDoc.PSDataSourcesConfig)) {
   Write-host "DatasourceConfig.xml file is not in the proper format."
   exit 1;
}

$exDSConfigList = @($psDSConfigDoc.PSDataSourcesConfig.ExchangeDatasourceConfig)
if(!$exDSConfigList) {
  write-host "There are no exchange datasources in the config file $configFilePath."
  exit 1;
}

if(!$source) {
	$source = read-host "Source FQDN"
}

if((!$source) -or ($source.split(".").count -lt 2)) {
	write-host -foregroundcolor Red "Invalid source FQDN $source !!!"
        exit 1;
}

$exServer = Get-ExchangeServer -Identity $source
if(!$exServer) {
	write-host -foregroundcolor Yellow "Exchange Server $source not found "
        $isContinue = Read-host "Do you want to continue and remove SG (Yes/No)? :"
	if($isContinue -ine "Yes") {
		write-host "Exiting"
		exit 1;
	}
}
...
if($exServer) {
	...
	if(!$sg) {
		write-host -foregroundcolor Yellow "Storage Group $sgName not found on server $source"
	        $isContinue = Read-host "Do you want to continue and remove SG (Yes/NO)? "
		if($isContinue -ine "Yes") {
			write-host "Exiting"
			exit 1;
...
### if SG is there, Remove the datasource entry based on SG GUID, 

if($sg) {
	$sgGuid = $sg.GUID.ToString()
	foreach($exDS in $exDSConfigList) {
		if($exDS.ComponentName -eq $sgGuid) {
			$psDSConfigDoc.PSDataSourcesConfig.RemoveChild($exDS)		
			...
			write-host "Successfully removed $source\$sgName ."
	                exit 0;
		} 
	}
	write-host -foregroundcolor Yellow "Mismatch in Storage group guids. Storage group, $sgName, may be removed and recreated after runing add-scrsg.ps1"
        write-host -foregroundcolor Yellow "The script will continue and try to remove it based on storage group name and source name."

} 

### If Either SG doesn't exist or SG Guids are different
### Remove SG entry based on Source name and SG name

foreach($exDS in $exDSConfigList) {
	if(($exDS.SourceServer -ieq $source) -and ($exDS.Caption -ieq $sgName)) {
		$psDSConfigDoc.PSDataSourcesConfig.RemoveChild($exDS)		
		$fullConfigFilePath =Convert-Path -literal $configFilePath
		write-host "Saving $fullConfigFilePath ."
		$psDSConfigDoc.save($fullConfigFilePath)
...
write-host "There is no datasource entry with source server: $source and SG: $sgName ."
exit 1;

# SIG # Begin signature block
# MIIbdgYJKoZIhvcNAQcCoIIbZzCCG2MCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQ