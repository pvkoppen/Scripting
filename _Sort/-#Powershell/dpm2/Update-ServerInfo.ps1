param([string] $DPMServerName, [string] $RebuiltServerName)

if(("-?","-help") -contains $args[0])
{
    	write-host "Usage::"
	write-host "Update-ServerInfo.ps1 -DPMServerName [DPM Server] -RebuiltServerName [Rebuilt Server]"
	write-host 
	write-host "Help: Use this script for the following scenarios to continue protection after"
	write-host "a) Rebuilding a DPM server"
	write-host "   Example: Update-ServerInfo.ps1 -DPMServerName dpm2 -RebuiltServerName dpm1"
	write-host "b) Rebuilding a protected computer"
	write-host "   Example: Update-ServerInfo.ps1 -DPMServerName dpm1 -RebuiltServerName ps1"
	write-host
	exit 0
}
if(!$DPMServerName)
...
if(!$RebuiltServerName)
{
	$RebuiltServerName = read-host "Rebuilt Server Name:"
}
$dpmobj = Get-ProductionServer $DPMServerName | where { ($_.machinename,$_.name) -contains $RebuiltServerName}
if(!$dpmobj)
{
	write-Error "Server::$RebuiltServerName Was Not Found"
	Disconnect-dpmserver
	exit 1
}	
$rebuiltServer = $dpmobj.name

#Running inquiry for the migrated PS to refresh volume guids
Write-Verbose "Refreshing Datasources On Server::$rebuiltServer"
$refreshDS = Get-Datasource $dpmobj -Inquire

if(!$refreshDS)
{
	write-Error "No Datasources Found For Server::$rebuiltServer"
	Disconnect-dpmserver
	exit 1
}
...
if(!$pglist)
{
	write-Error "No Active Protection Found On Server::$DPMServerName"
	Disconnect-dpmserver
	exit 1
}
foreach($pg in $pglist)
{
	$migrated = get-datasource -ProtectionGroup $pg | where { ($_.SourceDPMServer,$_.PSInfo.Name) -contains $rebuiltServer } 
	if($migrated)
	{
		$pgmod = Get-ModifiableProtectionGroup -ProtectionGroup $pg

		#Forcing intent translation for each migrated datasource
		Write-Verbose "Running Modify PG For PG::$pg.FriendlyName"

		Set-ProtectionGroup -ProtectionGroup $pgmod
	}
}	
Write-Host "Server Information Updated"
Disconnect-dpmserver

# SIG # Begin signature block
# MIIbdgYJKoZIhvcNAQcCoIIbZzCCG2MCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUyNLK72DcBrd0Cxz4LF3W6jD8
# 8eigghYqMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# KoZIhvcNAQkEMRYEFDVCOJ/O4WiBgS8PO6ik73+yZlFwMIGABgorBgEEAYI3AgEM
...
# d3cubWljcm9zb2Z0LmNvbS9EUE0wDQYJKoZIhvcNAQEBBQAEggEAG2VhVMm/+3mn
# DISo4qQUJ1Iru5KPsCqLLRnMcqfd5paEb8/DQpie0dQZB+fW4msF/RiweNDfU0BC
# LRuacRQ2MUzPOBF/I8BHFg4nRuPc+NWxoALcGpCo9ZbYVeS+ar+nCG6VT4oEjBC5
# /jYGc82MPKfYNkQhlQyQtwA3XkGGvxsbn+GlMae044DAw+/2dXIBh1zZUAbxFBGf
# dZ2JEpSQ6JCQwKFSfZjr/c13wUr1NZY0IkBJcQCvkFr7Is3jaxlWrjK9kQx5lEnN
# FG7ACY0PZviwnPxIYDyYSSgL9f51ZSPBX9DT0fgP1AWJeLxtTldm6CvzGfBTeb4p
# QC24Z00Fo6GCAh8wggIbBgkqhkiG9w0BCQYxggIMMIICCAIBATCBhzB5MQswCQYD
...
# OFowIwYJKoZIhvcNAQkEMRYEFCKF2KBC+Q3Juom2t2ThME/upTiZMA0GCSqGSIb3
# DQEBBQUABIIBAMXTyCMu8v4c+nysewdDSWDYs5VnXMhgkbkSR/X9aJL0YCRYwpdq
# hpibWBpKme8rvBaY7L4SkCmBqIJsEs/WwGoqagQoQwhCR/vfV7HHaqK7XoVAl4Vx
# 0cRZfv5QZAWGw3IoQzZN5k2nDfwhG+IpS7EzoftAFMLtrhhsiWZ2lUe0sFRudVSQ
# pqoqv5LdZX/uZWQTF21bZ+rJuOpF8Nh3JjI9J7++s5Yv3aWAjNy5IClRY5dw0Mig
# F+DSxWJ8+QOlhIET1wJB/b2AlEVHnEE0dlP/BIXRA2ATqT7qSh7GVISHQEQ6Jr4T
# rBfL/LhRWMj6SnP7bOCQPtvr8fzAamOTslM=
