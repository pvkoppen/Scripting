# Script to enable ExchangeSCRProtection property for a given protected computer
...
if(("-?","-help") -contains $args[0])
{
   write-host 
   write-host "Usage::"
   write-host "`tEnable-ExchangeSCRProtection.ps1 <DPMServerName> <ScrPSFQDN>" 
   write-host "`tEnable-ExchangeSCRProtection.ps1 <DPMServerName> <ResourceGroup>.<ClusterFQDN>"
   write-host 
   write-host "Example 1: "
   write-host "`tEnable-ExchangeSCRProtection.ps1 longhaulDPM  e12-vm.dpmvm.office" 
   write-host
   write-host "Example 2: "
   write-host "`tEnable-ExchangeSCRProtection.ps1 longhaulDPM  Lab-ccr-mbox.e12-vm.dpmvm.office" 
   write-host
   exit 0;
}
...
{
	Write-Error "SCR Protection is already enabled for $ScrPsFQDN"
	Disconnect-DPMServer 
	exit 1;
} else 
	{
		$ps = Get-productionServer -DPMServerName $dpmServerName | where {$_.name -ieq $ScrPsFQDN} 
		if(!$ps) 
		{
			Write-Error "Protected computer, $ScrPsFQDN, not found!!!"
			Disconnect-DPMServer 
			exit 1; 
		}
		...
		{
			write-host -foregroundcolor Red "You can not enable SCR protection for the protected computer until you remove all protected members on the associated computer from protection groups."
			write-host -foregroundcolor Red "Remove all of the members that are assosiated with protected computer $psName before enabling exchangeSCRProtection."
			Disconnect-DPMServer 
			exit 1;
		}
		...
		if($dsList.count -gt 0)
		{
			...
			write-host -foregroundcolor Red "Recovering from backups taken without SCR enabled is not supported after SCR enabling."
			Write-host -foregroundcolor Red "You need to disable SCR before recovering from these backups. Otherwise the recoveries are not guaranteed."
			$cont = Read-host "Do you want to continue enabling SCR(Yes\No)?"
		...
		if(!$scrList) 
		{
			...
			{
				$scrList = $scrList + "," + $ScrPsFQDN
		        }

		Set-DPMGlobalProperty -DPMServerName $dpmServerName -ExchangeSCRProtection $scrList
       }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUBpLaMHmz/bT+hHQ6gxbkQWhb
# Oj6gghYqMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# KoZIhvcNAQkEMRYEFCA1YymMnikpAMqgvKL+4A1vI/VxMIGABgorBgEEAYI3AgEM
...
# d3cubWljcm9zb2Z0LmNvbS9EUE0wDQYJKoZIhvcNAQEBBQAEggEACVK03JXiV5no
# k/bgR/h58eaUNV4CYDeng5hdFo6c3FMDoN4bYTaSZMSyj6A2AQ1XkH2kP+t1HpZ7
# SYaxTMXjgPAHgLplVsOZcXO4hcZX/tBRewJtd4SQ8KqRmpWdgSgAtvc9uLFQB2b4
# aKikrrymDhKdrADXjDLk7rhHcI6turx4ynOTpfWSadO8zPRpAdmZ2gBh1tDg947A
# RdV4os4Lt9rvhzC3HxFyBgJJQrueKXkasxJ/hr5wEjquL1XHff3miwyLeC5UjmSB
# s2o3m15e1AMk+qeCKCGIs0+KsqI6ytXmEB77qnSnf6sQsxRoXgpprmfR82wJl0sQ
# IK+qmKyY+6GCAh8wggIbBgkqhkiG9w0BCQYxggIMMIICCAIBATCBhzB5MQswCQYD
...
# OVowIwYJKoZIhvcNAQkEMRYEFHSCayhmdds237/T8jNfrK2hF6pMMA0GCSqGSIb3
# DQEBBQUABIIBABOGlWaP7Z0k0Tf1KOhs/MAl7AC/H1Ud5ITVbevoEpUYYnrqsP3e
# mo2cTAU8LJBGgsphson1/vhzR1pXBJ35xD7RJqOOiqfjvgCmqPQZlSynL5UCKIIa
# 063Ho+K5lteQIH1gaHxY9yrp8P9oYItXPUzSKEC7UtnuJv9X3jlhlwDpHK4teMpp
# i+lJ3vWKdPeiWcqWdJD8ca45mItxAEnGREfSz1ILqE/rmAtpQZxfj4BlfqhkHxSQ
# rXNHchko6Kyx7liOu+VATlb+ROAMvGmBEthgRzT//OdsIz+p2ZHMdROlz/gxJKzp
# pXtKrNXeqonBH/T4Wn03LTpYjsnKRTBO6dI=
