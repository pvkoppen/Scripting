...
if (("-?","-help") -contains $args[0]) {
  Write-Host "[INFO ] Usage: Mark-DPMTapeAsFree.ps1 [[-DPMServerName] <Name of the DPM server>] [-LibraryName] <Name of the library> [-TapeLocationList] <Array of tape locations>"
  Write-Host "[INFO ] Example: Mark-DPMTapeAsFree.ps1 -LibraryName ""My library"" -TapeLocationList Slot-1, Slot-7"
  exit 0
}
...
foreach ($media in @(Get-Tape -DPMLibrary $library)) {
  ...
  if ($TapeLocationList -contains $media.Location) {
    ...
    if ($media -is [Microsoft.Internal.EnterpriseStorage.Dls.UI.ObjectModel.LibraryManagement.ArchiveMedia]) {
      Write-Host "[INFO ] Removing recovery point(s) for tape in '$($media.Location)'."
      $FirstOne = $true
      foreach ($rp in @(Get-RecoveryPoint -Tape $media)) {
        Get-RecoveryPoint -Datasource $rp.Datasource | Out-Null
        Write-Verbose "[INFO ] Removing recovery point created at '$($rp.RepresentedPointInTime)' for tape in '$($media.Location)'."
        Remove-RecoveryPoint -RecoveryPoint $rp -ForceDeletion -Confirm:$false
      }
      Write-Host "[INFO ] Setting tape in '$($media.Location)' as free."
