...
# Check for DataSources that need to grow:
  ...
  foreach ($pgIndex in $pgAll) {
    ...
    $dsAll | convertto-html ProductionServerName,LogicalPath,ReplicaSize,RequiredReplicaSize,ReplicaUsedSpace,ShadowCopyAreaSize,RequiredShadowCopyAreaSize,ShadowCopyUsedSpace,ReplicaGrowRequired >> .\LogFiles\Report-DiskSpace-All.html #ProtectableObjectLoadPath
    $dsGrowRequired = $dsAll | ? {($_.ReplicaGrowRequired) -or (($_.ReplicaSize*0.75) -le $_.ReplicaUsedSpace)} 
    if ($dsGrowRequired) { 
      $dsGrowRequired | convertto-html ProductionServerName,LogicalPath,ReplicaSize,RequiredReplicaSize,ReplicaUsedSpace,ShadowCopyAreaSize,RequiredShadowCopyAreaSize,ShadowCopyUsedSpace,ReplicaGrowRequired >> .\LogFiles\Report-DiskSpace-SpaceRequired.html #ProtectableObjectLoadPath
    }
    $dsNotGrowRequired = $dsAll | ? {!($_.ReplicaGrowRequired) -and (($_.ReplicaSize*0.4) -ge $_.ReplicaUsedSpace)} 
    if ($dsNotGrowRequired) { 
      $dsNotGrowRequired | convertto-html ProductionServerName,LogicalPath,ReplicaSize,RequiredReplicaSize,ReplicaUsedSpace,ShadowCopyAreaSize,RequiredShadowCopyAreaSize,ShadowCopyUsedSpace,ReplicaGrowRequired >> .\LogFiles\Report-DiskSpace-OverAllocated.html #ProtectableObjectLoadPath
    }
  }
