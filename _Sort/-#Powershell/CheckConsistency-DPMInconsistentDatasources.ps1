...
if (("-?","-help") -contains $args[0]) {
  Write-Host "[INFO ] Usage: CheckConsistency-DPMInconsistentDatasources.ps1 [[-DPMServerName] <Name of the DPM server>]"
  Write-Host "[INFO ] Example: CheckConsistency-DPMInconsistentDatasources.ps1 -DPMServerName ""DPM01"" "
  exit 0
}
...
If ($pgAll) {
  foreach ($pgIndex in $pgAll) {
    ...
    $dsSelected = $dsAll | ? {$_.Protected -and $_.State -eq "Invalid" -and $_.Activity -eq "Idle"} 
