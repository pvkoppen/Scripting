' Name        : Kill-Process.vbs
' Description : Script used to kill a running process on the local computer.  (Needs access to WMI!)
...
If (intParameterCount = 0) Then
 WScript.Echo "Nothing to do: Script <process> [<process>]."
 WScript.Quit(0)
Else
  WScript.Echo "Running Command: Kill-Process.vbs <Proces(ses)="& WScript.Arguments(0) &" "& WScript.Arguments(1) &" ...>"
End If
