...
IF %USERDNSDOMAIN%' == ' SET InternalDomain=DMC.local
...
:ProcessEmail
REM -----------------------------------------------------------------------
"%~dp0BMail.exe" -s %SendMailServer% -t %3 -f %SendMailFrom% -h -a %4 -m %MailQueue%\\%3\\%4 -d
IF %ERRORLEVEL%' == 0' GOTO DelFile
ECHO [INFO] ERRORLEVEL=%ERRORLEVEL%.
GOTO End

:DelFile
ECHO [INFO] ERRORLEVEL=0: Delete File %MailQueue%\%3\%4
DEL %MailQueue%\%3\%4
GOTO End
