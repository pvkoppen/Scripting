...
"%IB_ProgramFolder%bin\gbak.exe" -C -V -Z -USER sysdba -PAS masterkey -Y "%MT32_BackupFolder%\Restore-MT32-%strTime%.Log" "%MT32_BackupFile%" "LocalHost:%MT32_InstallFolder%Data\MT32.IB" 
IF NOT %ERRORLEVEL%' == 0' SET strSendEmail=True
IF %strSendEmail%' == True' GOTO DoEnd
...
ECHO -----------------------------------------------------------------------
"%IB_ProgramFolder%bin\gbak.exe" -R -V -Z -USER sysdba -PAS masterkey -Y "%MT32_BackupFolder%\Restore-MT32-%strTime%.Log" "%MT32_BackupFile%" "LocalHost:%MT32_InstallFolder%Data\MT32.IB" 
IF NOT %ERRORLEVEL%' == 0' SET strSendEmail=True
IF %strSendEmail%' == True' GOTO DoEnd
START "Check Log" /WAIT "%MT32_BackupFolder%\Restore-MT32-%strTime%.Log"
GOTO DoEnd
