REM Activity      : Install APC PowerChute Network Shutdown x64
REM Created by    : Peter van Koppen
REM Company       : Tui Ora Limited
REM Date Created  : 2006-12-18
...
REM -----------------------------------------------------------------
%USE_DRIVE%
cd "\_usr.lib\Software\APCPowerChute_v2.2.1\x64"
for /d %%A in ("%SystemDrive%\Program*") do for /d %%B in ("%%A\java\jre*.*") do for /d %%C in ("%%B\bin") do %%C\java.exe -cp .;pcns221win64.jar;util.jar load -q %USE_DRIVE%\_usr.lib\Software\APCPowerChute_v2.2.1\x64\silentInstall.ini
...
pcns221win.exe -q "%USE_DRIVE%\_usr.lib\Software\APCPowerChute_v2.2.1\silentInstall.ini"
REM -----------------------------------------------------------------
