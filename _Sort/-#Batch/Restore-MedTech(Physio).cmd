...
:General2
SET MT32_BackupFile=%MT32_BackupFolder%pm0101-Physio.bak
...
Move /Y "%MT32_InstallFolder%Data\Physio.ib" "%MT32_BackupFolder%"
IF NOT %ERRORLEVEL%' == 0' SET strSendEmail=True
IF %strSendEmail%' == True' GOTO DoEnd
"%IB_ProgramFolder%bin\gbak.exe" -C -V -Z -USER sysdba -PAS masterkey -Y "%MT32_BackupFolder%\Restore-%strTime%-Physio.Log" "%MT32_BackupFile%" "LocalHost:%MT32_InstallFolder%Data\Physio.IB" 
IF NOT %ERRORLEVEL%' == 0' SET strSendEmail=True
IF %strSendEmail%' == True' GOTO DoEnd
START "Check Log" /WAIT "%MT32_BackupFolder%\Restore-%strTime%-Physio.Log"
GOTO DoEnd

:RestoreWithOverwrite
REM Restore Database: Overwrite file
"%IB_ProgramFolder%bin\gbak.exe" -R -V -Z -USER sysdba -PAS masterkey -Y "%MT32_BackupFolder%\Restore-%strTime%-Physio.Log" "%MT32_BackupFile%" "LocalHost:%MT32_InstallFolder%Data\Physio.IB" 
GOTO DoEnd
