...
FOR /D %%A IN (%striSCSIHome%%1*) DO SET strSRVFolder=%%A\
IF %strSRVFolder%' == ' GOTO ErrorNotFound
FOR /F %%A IN ('DIR /AD /B "%strSRVFolder%%1\Virtual Machines"') DO SET strVMCode=%%A
IF %strVMCode%' == ' GOTO ErrorNoVMCode

IF NOT EXIST "%SystemDrive%\ProgramData\Microsoft\Windows\Hyper-V\Virtual Machines\%strVMCode%.xml" mklink "%SystemDrive%\ProgramData\Microsoft\Windows\Hyper-V\Virtual Machines\%strVMCode%.xml" "%strSRVFolder%%1\Virtual Machines\%strVMCode%.xml"
ECHO.
...
ECHO.
icacls "%strSRVFolder%%1" /T /grant "NT VIRTUAL MACHINE\%strVMCode%":(F)
ECHO.
GOTO DoEnd
