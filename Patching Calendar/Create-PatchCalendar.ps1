...
#>
        ...
        Write-Host "RRULE:FREQ=DAILY;UNTIL=$(WriteDateTime ($script:dtStamp).AddDays($occurances))"
        Write-Host "SUMMARY:$($Subject)"
        Write-Host "TRANSP:OPAQUE"
        Write-Host "UID:MARCH-2020"
        Write-Host "LOCATION:Methanex NZ"
        Write-Host "DESCRIPTION:$($Subject)"
...
#>
    function AddCalendarDate($Start, $Finish, $Subject, $Instance) {
        ...
        Write-Host "DTSTAMP:$(WriteDateTime $script:dtStamp)"
        Write-Host "LAST-MODIFIED:$(WriteDateTime $script:dtStamp)"
        Write-Host "RECURRENCE-ID:$(WriteDateTime ($script:dtStamp).AddDays($Instance))"
...
#>
StartCalendar -Occurances 12 -Subject "Windows Patching $(($ProcessDate).ToString("yyyy-MM (MMM)"))"
AddCalendarDate -Instance 01 -Start ($SecondTuesday).AddHours(09) -Finish ($SecondTuesday).AddHours(09) -Subject "Patch Tuesday"
